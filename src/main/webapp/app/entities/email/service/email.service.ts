...
type RestOf<T extends IEmail | NewEmail> = Omit<T, 'date' | 'snoozedTo'> & {
  date?: string | null;
...
export class EmailService {
  ...
  protected convertDateFromClient<T extends IEmail | NewEmail | PartialUpdateEmail>(email: T): RestOf<T> {
    return {
      ...email,
      date: email.date?.toJSON() ?? null,
      snoozedTo: email.snoozedTo?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmail: RestEmail): IEmail {
    return {
      ...restEmail,
      date: restEmail.date ? dayjs(restEmail.date) : undefined,
      snoozedTo: restEmail.snoozedTo ? dayjs(restEmail.snoozedTo) : undefined,
    };
  }
