...
type RestOf<T extends ICertification | NewCertification> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class CertificationService {
  ...
  protected convertDateFromClient<T extends ICertification | NewCertification | PartialUpdateCertification>(certification: T): RestOf<T> {
    return {
      ...certification,
      date: certification.date?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCertification: RestCertification): ICertification {
    return {
      ...restCertification,
      date: restCertification.date ? dayjs(restCertification.date) : undefined,
    };
  }
