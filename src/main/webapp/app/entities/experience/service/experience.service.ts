...
type RestOf<T extends IExperience | NewExperience> = Omit<T, 'startDate' | 'endDate'> & {
...
export class ExperienceService {
  ...
  protected convertDateFromClient<T extends IExperience | NewExperience | PartialUpdateExperience>(experience: T): RestOf<T> {
    return {
      ...experience,
      startDate: experience.startDate?.format(DATE_FORMAT) ?? null,
      endDate: experience.endDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restExperience: RestExperience): IExperience {
    return {
      ...restExperience,
      startDate: restExperience.startDate ? dayjs(restExperience.startDate) : undefined,
      endDate: restExperience.endDate ? dayjs(restExperience.endDate) : undefined,
    };
  }
