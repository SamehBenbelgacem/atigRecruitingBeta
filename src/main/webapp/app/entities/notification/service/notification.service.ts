...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'callUpDate'> & {
...
export class NotificationService {
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      callUpDate: notification.callUpDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...restNotification,
      callUpDate: restNotification.callUpDate ? dayjs(restNotification.callUpDate) : undefined,
    };
  }
