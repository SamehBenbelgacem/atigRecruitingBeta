...
public class CompanyRepositoryWithBagRelationshipsImpl implements CompanyRepositoryWithBagRelationships {
    ...
    public Optional<Company> fetchBagRelationships(Optional<Company> company) {
        return company.map(this::fetchTags);
    }
    ...
    public List<Company> fetchBagRelationships(List<Company> companies) {
        return Optional.of(companies).map(this::fetchTags).orElse(Collections.emptyList());
    }

    Company fetchTags(Company result) {
        return entityManager
            .createQuery("select company from Company company left join fetch company.tags where company.id = :id", Company.class)
            .setParameter("id", result.getId())
            .getSingleResult();
    }

    List<Company> fetchTags(List<Company> companies) {
        ...
        List<Company> result = entityManager
            .createQuery("select company from Company company left join fetch company.tags where company in :companies", Company.class)
